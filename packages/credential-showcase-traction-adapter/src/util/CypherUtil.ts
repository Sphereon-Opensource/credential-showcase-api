...
function getKeyFromEnv(): Buffer {
  ...
  // Log warning if no key is set in environment
  console.warn('No encryption key found in environment variables. Using a temporary key for this session only.')
  // Generate a temporary key as Buffer directly
  return crypto.randomBytes(env.KEY_SIZE)
}
